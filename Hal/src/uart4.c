...
Uart4_Data_Struct  gl_Uart4Data;   //串口4全局数据
...
void uart4_Init(void)
{
	...
	USART_DMACmd(UART4, USART_DMAReq_Rx, ENABLE);          //使用DMA模式接收串口数据
	USART_DMACmd(UART4, USART_DMAReq_Tx, ENABLE);          //使用DMA模式发送串口数据
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (u32)gl_Uart4Data.rx_DataBuff; // 存储器0地址 
	...
	DMA_InitStructure.DMA_Channel = DMA_Channel_4;
	DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&UART4->DR; // 外设地址
	DMA_InitStructure.DMA_Memory0BaseAddr = (u32)gl_Uart4Data.tx_DataBuff; // 存储器0地址 
	DMA_InitStructure.DMA_BufferSize = 0;   // 数据传输量
...
void UART4_IRQHandler(void)
{
	...
	if(USART_GetITStatus(UART4, USART_IT_IDLE)!=RESET)    //空闲中断产生 一帧已经传输完成
	{ 
		temp=UART4->SR;
		temp=UART4->DR;  //由软件序列清除IDLE标记 详见中文参考手册
		DMA_Cmd(DMA1_Stream2, DISABLE);   //关闭DMA通道
		DMA_ClearFlag(DMA1_Stream2, DMA_FLAG_TCIF2);//清除传输完成标记
		DMA_ClearFlag(DMA1_Stream2, DMA_FLAG_TEIF2);//清除传输错误标记
		...
		DMA_SetCurrDataCounter(DMA1_Stream2, UART4_RXBUFF_SIZE);//设置传输长度
		DMA_Cmd(DMA1_Stream2, ENABLE);   //开启DMA通道
	}

	if(USART_GetITStatus(UART4, USART_IT_TC)!=RESET)
	{
		USART_ClearITPendingBit(UART4, USART_IT_TC);
	    DMA_ClearFlag(DMA1_Stream4, DMA_FLAG_TCIF4); // 传输完成标志	    
	    DMA_Cmd(DMA1_Stream4, DISABLE);
	    RS485_HOST_DIR_EN=1;   //设置为接收模式
	}
}
...
void DMA1_Stream4_IRQHandler(void)
{
	...
	{
	    DMA_ClearFlag(DMA1_Stream4, DMA_FLAG_TCIF4 | DMA_FLAG_FEIF4); // 传输完成标志
